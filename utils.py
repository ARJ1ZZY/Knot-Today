...
def draw_soft_rect(surface, rect, color, radius, border_width=1, border_color=None, shadow=True):
    if shadow:
    ...
    main_surf = pygame.Surface((rect.width, rect.height), pygame.SRCALPHA)
    pygame.draw.rect(main_surf, color, (0, 0, rect.width, rect.height), border_radius=radius)
    
    if border_width > 0:
        if border_color is None:
            border_color = (255, 220, 190, 100)
        pygame.draw.rect(main_surf, border_color, (0, 0, rect.width, rect.height), border_width, border_radius=radius)
    
    surface.blit(main_surf, rect.topleft)

def draw_glass_rect(surface, rect, color, radius, border_width=1, border_color=None, shadow=True):
    draw_soft_rect(surface, rect, color, radius, border_width, border_color, shadow)
